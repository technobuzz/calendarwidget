...
// modified 7/24 to compensate for restyle & better UI placement of Calendar - Brian
	o.div.style.left= (pos[0]-126)+"px";
//alert("Left: "+osw+" "+o.div.style.left);
...
// Control btns row
                        ...
                        setARIARole(tr, "presentation");
                        tableHead.appendChild(tr);
						tr.addClass="ctrlBtns";


/* Original Unicode characters for year changes
...
// Keyboard Date Assignment
                this.onkeydown = function (e) {
                        ...
                        if( kc == 13 ) {	// CRLF:close & select the date
                                var td = document.getElementById(o.id + "-date-picker-hover");                                         
						...
						else if(kc == 27) {	// ESC:close, no date selection
                                if(!o.staticPos) {
									o.hide();
									return stopEvent(e);
                                };
						...
						else if(kc == 32 || kc == 0) {	// SPACE:goto today's date 
                                o.date = new Date();
                                o.updateTable();
                                return stopEvent(e);
                        }
						else if(kc == 9) {	// TAB:close - no date selection - remove focus on button - popup ver only                   
                                if(!o.staticPos) {
                                    o.hide();
                                    return stopEvent(e);
                                };
                        ...
                        if(kc == 36) {		// HOME:Set date to first day of current month
                        	d.setDate(1); 
                        }
						else if(kc == 35) {	// END:Set date to last day of current month  
                                d.setDate(daysInMonth(d.getMonth(),d.getFullYear())); 
                        }
						else if ( kc == 33 || kc == 34) {	// PAGE UP & DOWN   
                                var inc = (kc == 34) ? 1 : -1; 
							if(e.ctrlKey) {	 // CTRL + PAGE UP/DOWN: Move to same date in prev/next year                                                                   
                                        d.setFullYear(d.getFullYear() + inc * 1);        
							}
							else {		// PAGE UP/DOWN:Moves to the same date in the previous/next month
									var n = o.date.getDate();                         
									d.setDate(2);
									d.setMonth(d.getMonth() + inc * 1);
									d.setDate(Math.min(n, daysInMonth(d.getMonth(),d.getFullYear())));                                         
						...
						else if ( kc == 39 || kc == 34) {	// RIGHT ARROW
							d = new Date(o.date.getFullYear(), o.date.getMonth(), o.date.getDate() + 1 ); 
                       	}
...
//		alert(this.bespokeTabIndex);	// being set but ignored when tabbing??
                };                              
        };
...
// Converts $date to digits w/ separator
        datePicker.prototype.returnFormattedDate = function(noFocus) {     
                ...
                for(elemID in this.formElements) {
                        ...
                        fmtDate = printFormattedDate(this.dateSet, elemFmt, returnLocaleDate);
							var DaTe = fmtDate.substring(0,2)
							var MnTh = fmtDate.substring(3,5);
// Convert 2 digit Month back to 3-char string
		switch(MnTh) {
			case '01':
				MnTh='Jan';
				break;
			...
			case '12':
				...
				break;
							...
							var YeAr = fmtDate.substring(6,8)
							YeAr = "20"+YeAr;
						fmtDate = (DaTe+"-"+MnTh+"-"+YeAr);
...
var opts = {
		...
		id:options.id,	// Form element id
		formatMasks:options.formatMasks, // Format masks 
		staticPos:!!(options.staticPos), // Non popup datepicker required
		...
		dragDisabled:nodrag || !!(options.staticPos) ? true : !!(options.dragDisabled), // No drag functionality
		bespokeTabIndex:options.bespokeTabindex && typeof options.bespokeTabindex == 'number' ? parseInt(options.bespokeTabindex, 10) : 0,
		bespokeTitles:options.bespokeTitles || {},
		finalOpacity:options.finalOpacity && typeof options.finalOpacity == 'number' && (options.finalOpacity > 20 && options.finalOpacity <= 100) ? parseInt(+options.finalOpacity, 10) : (!!(options.staticPos) ? 100 : finalOpacity),
		hideInput:!!(options.hideInput),	// Hide form elements on datepicker creation ?
		noToday:!!(options.noTodayButton),	// Hide "today" button ?
		showWeeks:!!(options.showWeeks), 	// Do we show week numbers
		fillGrid:!!(options.fillGrid),		// Fill the entire grid with dates  
